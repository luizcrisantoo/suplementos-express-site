...
export default async function PaginaProduto({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    name: p.nome, brand: { '@type': 'Brand', name: p.marca }, description: p.descricao ?? undefined,
    offers: {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div className="relative aspect-square rounded-card border border-neve-200 bg-neve-200/30">
          {p.foto && <Image src={`/produtos/${p.foto}`} alt={p.nome} fill priority
                            sizes="(max-width:1024px) 100vw, 500px" className="object-contain p-8" />}
        </div>

        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-neve-600">{p.marca}</p>
          <h1 className="mt-1 font-display text-3xl font-extrabold leading-tight">{p.nome}</h1>
          ...
          <p className="mt-1 text-sm text-ok">
            No Pix. Em até 6x sem juros no cartão.
          </p>

          <p className="mt-4 inline-flex items-center gap-2 rounded-lg bg-ouro-100 px-3 py-2 text-sm font-semibold text-ouro-700">
            {p.disponibilidade === 'pronta_entrega'
              ? 'Pronta entrega: pediu até 16h, recebe hoje'
              : 'Sob encomenda: chega em até 24h'}
          </p>

          <BotaoComprar produto={{ id: p.id, slug: p.slug, nome: p.nome, preco_cents: p.preco_venda_cents, foto: p.foto }} />
